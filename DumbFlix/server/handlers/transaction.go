...
func dueDate(startTime time.Time, duration time.Duration) time.Time {
...
func (h *TransactionHandler) CreateTransaction(c echo.Context) error {
	...
	startTime := time.Now()
	duration := time.Hour * 24 // atur durasinya sih
	dueDate := dueDate(startTime, duration)
...
func SendMail(status string, transaction models.Transaction) {
	...
	if status != transaction.Status && (status == "success") {
		...
		productName := transaction.Product.Duration
